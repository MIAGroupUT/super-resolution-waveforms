function pulseOUT = pulsePropagation(pulseIN, liquid, bubble)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% FREQUENCIES
f0 = pulseIN.f0/1e6;         % centre frequency (MHz)
...
dim = 2;     % Compute the fourier transform per row.
pulseFFT = fft(pulseIN.p,[],dim);
% Compute attenuated amplitude spectrum
p1FFT = pulseFFT.*10.^(-d*alpha*100/20);
% Convert back to time domain
p1 = real(ifft(p1FFT,[],dim));  
...
pulseOUT.f0 = pulseIN.f0;
pulseOUT.w  = pulseIN.w;
pulseOUT.dispProgress = pulseIN.dispProgress;
