...
pulse.f0 = 1.7e6;           % Driving frequency (Hz)
pulse.w = pulse.f0*2*pi;    % Angular frequency (Hz)

% Time vector pulse:
pulse.t = 0:1/pulse.fs:(pulse.Nc/pulse.f0 + pulse.Tresp);

% Pulse with Nc cycles and Hann window:
pulse.p = pulse.A*sin(2*pi*pulse.f0*pulse.t)... 
    .*sin(pi*pulse.f0/pulse.Nc*pulse.t).^2;
pulse.p(pulse.t>pulse.Nc/pulse.f0) = 0;
